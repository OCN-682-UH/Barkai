...
library(janitor) #Clean up the column headers with this
```
...
algae_data <- Data %>% #Using my dataset called Data and making a new name
  ...
  mutate(Site = fct_reorder(Site, Latitude)) # Reorder the latitude to south → north bc fct_reorder-factor_reorder goes from lowest to highest and this is telling it to do it with the site latitude from lowest to highest
...
#Cleaning up the column headers with stuff learned from working with words lesson
mean_summary <- mean_summary %>%
  rename(Mean_Algae_Cover = mean_cover) %>% #Rename mean_cover to Mean_Algae_Cover so you know what it is and be caps too
  mutate(Site = str_trim(Site),  #Removes spaces on left and right
         Site=str_to_title(Site)) #Make sure it is capitilized
```
...
mean_plot <- mean_summary %>% #Name for plot
  ggplot(aes(x = Site, y = Mean_Algae_Cover)) + #X and y axis
  geom_col(fill = "seagreen3") + #color of bars
  geom_text(aes(label = round(Mean_Algae_Cover, 1), #Rounds the cover to one decimal place
                color = ifelse(Mean_Algae_Cover == min(Mean_Algae_Cover), "low", #finding the lowest number
                               ifelse(Mean_Algae_Cover == max(Mean_Algae_Cover), "high", "normal"))), #finding the highest number
